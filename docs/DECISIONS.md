...
Server compatibility uses a versioned operational tuple of the three records, including Signet and excluding explorer URLs. The public wire format encodes bigint chain IDs as canonical decimal strings. Server configuration and funding authority remain owned by the server environment.

Verification includes network/default and package-publication tests, immutable owner and editor supersession tests, canonical server DTO tests, independent EVM wallet connection and RPC mismatch tests. Public HTTP and WebSocket transport probes establish endpoint availability separately from funded contract acceptance.
