...
/** @type {[string, (source: string) => string][]} */
const cases = [
  [
    "state palette",
    (value) => value.replace("<form", "<form className='data-[state=open]:bg-red-500'"),
  ],
  ["arbitrary CSS", (value) => value.replace("<form", "<form className='[color:red]'")],
  ["aliased Toaster", (value) => `import { Toaster as Notifications } from 'sonner';\n${value}`],
  [
    "aliased primitive",
    (value) =>
      value
  ...
  ["inline style", (value) => value.replace("<form", "<form style={{ color: 'red' }}")],
  ["unknown recipe", (value) => value.replace("<form", "<form className='ds-unregistered'")],
  ["primitive override", (value) => value.replace("<Input", "<Input className='ds-caption'")],
...
try {
  for (const [name, mutate] of cases) {
    ...
    assert.notEqual(mutated, original, `${name} must plant a source change`);
    writeFileSync(target, mutated);
    const result = spawnSync(process.execPath, ["scripts/design-system/check.mjs"], {
      cwd: root,
      encoding: "utf8",
    });
