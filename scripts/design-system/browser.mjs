export async function verifyDesignSystem(page) {
  ...
  await page
    .getByRole('heading', { name: 'Component states (development fixtures)' })
    .waitFor();
  ...
  const contrasts = await page.evaluate(() => {
    ...
    const values = [
      ...
      return [text, surface, (Math.max(a, b) + 0.05) / (Math.min(a, b) + 0.05)];
    });
    probe.remove();
    return values;
  });
  ...
  for (const variant of [
    ...
    assert(
      ...
      `${variant} button hover is visible`,
    );
    await button.focus();
    ...
    assert(
      focus.focusVisible &&
        (focus.shadow !== 'none' ||
          !['none', 'hidden'].includes(focus.outline)),
  ...
  assert(
    ...
    'Disabled button has native disabled semantics',
  );
  assert(
  ...
  assert(
    ...
    'Menu input set is non-empty',
  );
  const menuGuard = async () => {
    if ((await styles(menu)).cursor !== 'pointer')
      throw new Error('Menu pointer regressed');
  };
  await menuGuard();
  ...
  assert(
    caught,
    'Planted menu cursor regression fails the computed-style guard',
  );
  await menuGuard();
  ...
  assert(
    composition.cursor === 'pointer' &&
      composition.background === highlighted.background,
  ...
  assert(
    await amounts.nth(2).isDisabled(),
    'Disabled amount is natively disabled',
  );
