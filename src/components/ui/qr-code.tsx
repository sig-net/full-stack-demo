...
async function convertIconToDataUrl(icon: ReactElement): Promise<string | undefined> {
  try {
    const tempDiv = document.createElement("div");
    tempDiv.style.cssText = "position:absolute;left:-9999px;width:64px;height:64px";
...
/**
 * Prevents superseded asynchronous QR generation from publishing into the container.
 *
 * @param properties - QR value, dimensions, correction level and optional icon.
 * @returns The QR container, loading state or generation error.
 */
export function QRCode(properties: QRCodeProps): React.JSX.Element {
  const {
    value,
    size = 160,
    className,
  ...
  if (error) {
    return (
      ...
      >
        <div className="ds-caption ds-label ds-error text-center">QR Code Error</div>
        <div className="ds-caption ds-error ds-before-control text-center">{error}</div>
      </div>
    );
  }
