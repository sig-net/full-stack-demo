...
const buttonVariants = cva(
  "group/button cursor-pointer inline-flex shrink-0 items-center justify-center rounded-lg border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:cursor-not-allowed disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      ...
      size: {
        row: "h-auto w-full justify-between whitespace-normal p-3 text-left",
        default:
...
/**
 * Delegates DOM props and ref handling to the child when `asChild` is enabled.
 * The `menu` variant relies on its enclosing dropdown item for appearance and focus.
 *
 * @param properties - Native button properties and bounded visual variants.
 * @returns The button element or composed child.
 */
function Button(
  ...
  return (
    <Comp
      ...
      data-size={size}
      className={cn(
        variant === "menu" ? "w-full justify-start" : buttonVariants({ variant, size }),
        className,
      )}
      {...props}
    />
